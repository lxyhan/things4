...
export function ProjectDetail({ projectId }: ProjectDetailProps): React.JSX.Element {
  ...
  useEffect(() => {
    window.api?.projects?.get(projectId).then((p) => {
      if (!p) return
      setProject(p)
  ...
  function handleTitleCommit(): void {
    ...
    window.api?.projects?.update(projectId, { title: titleDraft })
      .then((updated) => { if (updated) setProject(updated) })
      .catch(() => undefined)
  }

  function handleNotesBlur(): void {
    if (!project || notesDraft === (project.notes ?? '')) return
    window.api?.projects?.update(projectId, { notes: notesDraft || null })
      .then((updated) => { if (updated) setProject(updated) })
      .catch(() => undefined)
  }
  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        ...
        {totalTasks > 0 && (
          <div className={styles.progressArea} title={`${completedTasks} of ${totalTasks} done`}>
