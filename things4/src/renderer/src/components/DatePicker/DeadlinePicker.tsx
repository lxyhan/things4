...
export function DeadlinePicker({ value, onChange, onClose, anchorEl }: DeadlinePickerProps): React.JSX.Element {
  const today = getToday()
  ...
  const handleContainerKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowLeft') { newDate.setDate(newDate.getDate() - 1); e.preventDefault() }
    else if (e.key === 'ArrowRight') { newDate.setDate(newDate.getDate() + 1); e.preventDefault() }
    else if (e.key === 'ArrowUp') { newDate.setDate(newDate.getDate() - 7); e.preventDefault() }
    else if (e.key === 'ArrowDown') { newDate.setDate(newDate.getDate() + 7); e.preventDefault() }
    else return
  ...
  const handleNLKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'Enter' && nlInput.trim()) {
      ...
      if (parsed) { onChange(toISO(parsed)); onClose() }
      e.stopPropagation()
    }
  }

  const isISO = value && /^\d{4}-\d{2}-\d{2}$/.test(value)
  const selectedISO = isISO ? value : null
  const focusedISO = toISO(focusedDate)
  const todayISO = toISO(today)
  const urgency = isISO ? daysUntilLabel(value!) : null
  const isOverdue = isISO && new Date(value! + 'T00:00:00') < today
  ...
  const picker = (
    ...
    >
      {urgency && (
        <div className={`${styles.urgencyBanner} ${isOverdue ? styles.urgencyOverdue : styles.urgencyNormal}`}>
          {urgency}
        </div>
      )}
      ...
      <div className={styles.monthHeader}>
        <button className={styles.navBtn} onMouseDown={e => { e.preventDefault(); prevMonth() }}>‹</button>
        <span className={styles.monthLabel}>{MONTHS[viewMonth]} {viewYear}</span>
        <button className={styles.navBtn} onMouseDown={e => { e.preventDefault(); nextMonth() }}>›</button>
      </div>
        ...
        {cells.map((d, i) => {
          ...
          return (
            <button
              ...
              onMouseDown={e => { e.preventDefault(); selectDate(d) }}
            >
              {d.getDate()}
            </button>
          )
