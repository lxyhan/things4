...
export function TagPicker({ selectedIds, onChange, onClose, anchorEl }: TagPickerProps): React.JSX.Element {
  ...
  const createTag = useCallback(async () => {
    ...
    try {
      const newTag = await window.api?.tags?.create({ name: query.trim(), color: null, shortcut: null })
      if (newTag) {
        ...
        {filtered.map((tag, idx) => {
          ...
          return (
            ...
            >
              ...
              {tag.shortcut && <span className={styles.shortcut}>{tag.shortcut}</span>}
