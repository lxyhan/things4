...
export function registerViewTools(server: McpServer, db: Database.Database): void {
  ...
  server.tool(
    ...
    async () => {
      ...
      if (count === 0) {
        ...
        suggestions.push(`You have ${count} items in your inbox. Time for an inbox processing session.`)
        suggestions.push('For each item: assign to a project/area, set a when_date, or move to someday.')
      } else {
        suggestions.push(`${count} item${count === 1 ? '' : 's'} waiting to be processed.`)
      }
      ...
      if (oldest) {
        ...
        if (ageDays >= 7) {
          suggestions.push(`Oldest item is ${ageDays} days old: "${oldest.title}". Consider processing it now.`)
        }
      }
  ...
  server.tool(
    ...
    async () => {
      ...
      const allActiveProjects = db.prepare(`
