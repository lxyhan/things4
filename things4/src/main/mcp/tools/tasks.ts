...
export function registerTaskTools(server: McpServer, db: Database.Database): void {
  ...
  server.tool(
    ...
    async ({ title, notes, project_id, area_id, when_date, deadline, waiting_for, waiting_since, tags }) => {
  ...
  server.tool(
    ...
    async ({ id, project_id, area_id }) => {
      ...
      `).run(
        project_id !== undefined ? project_id : (task as { project_id: string | null }).project_id,
        area_id !== undefined ? area_id : (task as { area_id: string | null }).area_id,
        now,
        id
      )
