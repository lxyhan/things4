...
const ACTION_VERBS = new Set([
  'add', 'analyze', 'approve', 'ask', 'attend', 'book', 'build', 'buy', 'call',
  'cancel', 'check', 'clean', 'close', 'collect', 'compile', 'complete', 'confirm',
  'connect', 'contact', 'copy', 'create', 'debug', 'define', 'delete', 'deploy',
  'design', 'discuss', 'do', 'document', 'download', 'draft', 'edit', 'email',
  'explore', 'file', 'fill', 'find', 'finish', 'follow', 'forward', 'get',
  'implement', 'install', 'investigate', 'look', 'make', 'meet', 'move', 'notify',
  'open', 'order', 'organize', 'pay', 'pick', 'plan', 'prepare', 'print',
  'process', 'publish', 'read', 'record', 'refactor', 'remind', 'remove', 'reply',
  'report', 'request', 'research', 'respond', 'review', 'run', 'schedule', 'search',
  'send', 'set', 'share', 'sign', 'start', 'submit', 'sync', 'test', 'transfer',
  'update', 'upload', 'verify', 'write',
])
...
function analyzeGtd(raw: string): GtdAnalysis {
  ...
  if (MULTI_STEP_PATTERN.test(title) || isLong) {
    clarifications.push('This sounds like a project. What is the first physical next action?')
  }

  // Rule: deadline vs when_date ambiguity
  const hasDeadlinePhrase = /\bby\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|\d{1,2}[\/\-]\d)/i.test(raw)
  const hasStartPhrase = /\b(on|starting|from)\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday|\d{1,2}[\/\-]\d)/i.test(raw)
...
export function registerCaptureTools(server: McpServer, db: Database.Database): void {
  server.tool(
    'capture',
    'GTD-enforcing intake: parses raw text, enforces next-action rules, and captures to inbox',
    {
      ...
      context: z.string().optional().describe('Optional context (e.g., "in a meeting with Alice")'),
    },
